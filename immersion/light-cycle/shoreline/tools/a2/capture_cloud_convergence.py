...
def main():
    ...
    html=(ROOT/'Open_Moon_Shoreline.html').read_bytes();html_sha=hashlib.sha256(html).hexdigest();profiles=json.loads((ROOT/'data/a1/generated/profiles.json').read_text());profile=next(x for x in profiles['profiles'] if x['id']=='moon-fair')
