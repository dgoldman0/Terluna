...
def main():
    ...
    for path,digest in manifest['sources'].items():assert sha(ROOT/path)==digest, 'Stale profile source: '+path
    ...
    browser=read(ROOT/'validation/a1/lab-browser.json');assert browser['passed'] and not browser['errors'] and not browser['external_requests'];assert browser['html_sha256']==sha(ROOT/'Open_Moon_A1_Accuracy_Lab.html')
    assert sha(ROOT/'Open_Moon_Shoreline.html')==BASELINE_HTML
    ...
    paths=[ROOT/'build_accuracy.py',ROOT/'index.accuracy-lab.html',ROOT/'src/accuracy-lab.js',ROOT/'src/atmospheric-profile.js',ROOT/'src/profile-optics.js',ROOT/'src/frozen-cloud-field.js',ROOT/'src/weather-column.js',ROOT/'src/cloud-renderer.js',ROOT/'tests/a1_frozen_probe.js',ROOT/'tests/atmospheric-profile.test.cjs',ROOT/'tests/frozen-cloud-field.test.cjs',ROOT/'data/a1/spectral-inputs.json',ROOT/'data/a1/BRUNETON_LICENSE.txt',ROOT/'A1_METHODS.md',ROOT/'A1_THIRD_PARTY_NOTICES.md',ROOT/'requirements-a1.txt',*sorted((ROOT/'tools/a1').glob('*.py')),*sorted((ROOT/'tools/a1').glob('*.cjs'))]
    ...
    'baseline_html_sha256':BASELINE_HTML,'source_sha256':{str(p.relative_to(ROOT)):sha(p) for p in paths},'runtime':{'node':subprocess.check_output(['node','--version'],text=True).strip(),'python_packages':{x:importlib.metadata.version(x) for x in ['numpy','scipy','numba','playwright']}},
    ...
    'generated_artifacts':{p.name:{'bytes':p.stat().st_size,'sha256':sha(p)} for p in [ROOT/'Open_Moon_A1_Accuracy_Lab.html',GEN/'profiles.json',GEN/'moon-fair.gpu.json',GEN/'moon-fair.mc.json',GEN/'moon-fair.cloud.json']}}
