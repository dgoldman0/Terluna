...
def main():
 ...
 inputs=read(v/'input-integrity.json');gate('Inherited generated input integrity',inputs['passed'] and all(sha(ROOT/x['path'])==x['sha256'] for x in inputs['files']),files=len(inputs['files']));report['input_dependencies']=inputs['files']
 ...
 for p in sorted((gen/'maps').glob('*.json')):
  ...
  maphash&=sha(p.with_suffix('.npz'))==m['output_sha256'];mapidentity&=all(sha(ROOT/name)==value for name,value in m['source_hashes'].items())
 ...
 gate('Production shoreline build preserved',sha(ROOT/'Open_Moon_Shoreline.html')==BASELINE_HTML,sha256=sha(ROOT/'Open_Moon_Shoreline.html'))
