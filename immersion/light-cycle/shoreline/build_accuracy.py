...
def build(data_dir:Path,output:Path)->None:
    ...
    for r in manifest['profiles']:
        ...
        for key,path in [('solver_sha256',ROOT/'tools/a1/reference_optics.py'),('spectral_sha256',ROOT/'data/a1/spectral-inputs.json')]:
    ...
    if data['cloudMC']['solver_sha256']!=hashlib.sha256((ROOT/'tools/a1/cloud_reference.py').read_bytes()).hexdigest():raise ValueError('Stale cloud reference')
    payload=json.dumps(data,separators=(',',':'),ensure_ascii=True).replace('</','<\\/')
    html=(ROOT/'index.accuracy-lab.html').read_text().replace('__A1_DATA__',payload).replace('__A1_SCRIPT__',(ROOT/'src/accuracy-lab.js').read_text()).replace('__A1_LICENSE__',html_module.escape((ROOT/'data/a1/BRUNETON_LICENSE.txt').read_text()))
...
if __name__=='__main__':
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--data',type=Path,default=ROOT/'data/a1/generated');ap.add_argument('--output',type=Path,default=ROOT/'Open_Moon_A1_Accuracy_Lab.html');a=ap.parse_args();build(a.data,a.output)
