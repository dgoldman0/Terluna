...
def next_collision(p,d,R,top,rows,oz,beta,sigma,edges,air_bounds,oz_bounds,lo,hi,dims,ext,ice,cmax,calbedo,gas_on,cloud_on):
    ...
    for j in range(1,len(ts)):
        ...
        M=segment_majorant(p+d*((a+b)/2),R,edges,air_bounds,oz_bounds,beta,sigma,lo,hi,cmax,gas_on,cloud_on)
        if M<=0:continue
        t=a
        while trials<100000:
            trials+=1;t+=-math.log(max(1e-300,np.random.random()))/M
            if t>=b:break
            q=p+d*t;gs,ga,c,ic=coefficients(q,R,rows,oz,beta,sigma,lo,hi,dims,ext,ice,gas_on,cloud_on)
            if gs+ga+c>M*(1+1e-10):return q,-1,ic,ground,trials
            u=np.random.random()*M
            if u<gs:return q,1,0.,ground,trials
            if u<gs+c*calbedo:return q,2,ic,ground,trials
            if u<gs+ga+c:return q,3,ic,ground,trials
...
def transmission(p,d,R,top,rows,oz,beta,sigma,edges,air_bounds,oz_bounds,lo,hi,dims,ext,ice,cmax,gas_on,cloud_on,ground_blocks=True):
    """Unbiased ratio tracking through both media; no optical lookup table."""
    ts,ground=segments(p,d,R,top,edges,lo,hi,cloud_on)
    ...
    for j in range(1,len(ts)):
        ...
        while trials<100000:
            trials+=1;t+=-math.log(max(1e-300,np.random.random()))/M
            if t>=b:break
            gs,ga,c,ic=coefficients(p+d*t,R,rows,oz,beta,sigma,lo,hi,dims,ext,ice,gas_on,cloud_on)
            if gs+ga+c>M*(1+1e-10):return 0.,1
            tr*=max(0.,1-(gs+ga+c)/M)
            if tr<1e-5:
