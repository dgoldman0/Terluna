...
function noiseLattice(n=64){
 ...
 for(let z=0;z<n;z++)for(let y=0;y<n;y++)for(let x=0;x<n;x++){
  let h=Math.imul(x+17,374761393)^Math.imul(y+31,668265263)^Math.imul(z+79,2147483647);
  h=Math.imul(h^(h>>>13),1274126177);bytes[(z*n+y)*n+x]=(h^(h>>>16))>>>24;
 }
 return bytes;
}
const DENSITY_GLSL=`
...
class Controller{
 constructor(T,atm){
  this.T=T;this.atm=atm;this.key='reference';this.cache=new Map();this.current=null;this.world=null;this.renderKey=null;this.bakeCount=0;this.lastBakeMs=0;
  const texture=(data,w,h)=>{const t=new T.DataTexture(data,w,h,T.RGBAFormat,T.FloatType);t.minFilter=t.magFilter=T.LinearFilter;t.generateMipmaps=false;t.needsUpdate=true;return t;};
  this.noiseTexture=new T.Data3DTexture(noiseLattice(),64,64,64);this.noiseTexture.format=T.RedFormat;this.noiseTexture.type=T.UnsignedByteType;this.noiseTexture.minFilter=this.noiseTexture.magFilter=T.LinearFilter;this.noiseTexture.wrapS=this.noiseTexture.wrapT=this.noiseTexture.wrapR=T.RepeatWrapping;this.noiseTexture.unpackAlignment=1;this.noiseTexture.needsUpdate=true;
  this.profileTexture=texture(new Float32Array(256*4),256,1);this.sunTexture=texture(new Float32Array(128*64*4),128,64);
  Object.assign(atm.uniforms,{uColumnNoiseTexture:{value:this.noiseTexture},uColumnCacheReady:{value:0},uColumnShadowReady:{value:0},uColumnAverageShadow:{value:1},uColumnSkyMap:{value:null},uColumnShadowMap:{value:null},uColumnShadowBounds:{value:new T.Vector4(-16384,-16384,32768,128)},uColumnMode:{value:0},uColumnBase:{value:0},uColumnTop:{value:1},uColumnScale:{value:10000},uColumnCoverage:{value:.4},uColumnSteps:{value:128},uColumnTime:{value:0},uOpticalH:{value:48400},uOpticalDensity:{value:1.2},uOpticalBeta:{value:new T.Vector3(...O.BETA)},uLightTop:{value:65000},uColumnSolar:{value:new T.Vector3(1,1,1)},uColumnFogColour:{value:new T.Vector3()},uColumnProfile:{value:this.profileTexture},uColumnSun:{value:this.sunTexture}});
 }
 select(key){if(key!=='reference'&&!W.PRESETS[key])throw new RangeError('Unknown atmospheric study');this.key=key;this.world=null;this.current=null;this.atm.envTime=-1e9;this.atm.clearEnvKey=null;this.atm.uniforms.uColumnMode.value=0;this.atm.uniforms.uColumnCacheReady.value=0;this.atm.uniforms.uColumnShadowReady.value=0;this.renderKey=null;}
 update(world){
  const u=this.atm.uniforms;if(this.key==='reference'){u.uColumnMode.value=0;return;}
  if(this.world!==world||!this.current){
   const key=world+':'+this.key;let record=this.cache.get(key);
   if(!record){const model=W.create(this.key,world),profile=W.textureData(model),light=O.buildTable(world,profile.top+1000);record={model,profile,light};this.cache.set(key,record);}
   this.current=record;this.world=world;this.profileTexture.image.data=record.profile.data;this.profileTexture.needsUpdate=true;this.sunTexture.image.data=record.light.data;this.sunTexture.needsUpdate=true;
   u.uColumnBase.value=record.profile.base;u.uColumnTop.value=record.profile.top;u.uLightTop.value=record.light.maxHeight;
   const m=record.model.inputs.morphology;u.uColumnScale.value=Math.max(300,(record.profile.top-record.profile.base)*m.aspect*3.2);u.uColumnCoverage.value=m.coverage;
   const optical=O.profile(world),t=O.transmission(world,0,1);u.uOpticalH.value=optical.H;u.uOpticalDensity.value=optical.density;
   ...
   u.uColumnSolar.value.set(...this.atm.data.worlds[world].direct.at(-1).map((e,i)=>Math.max(0,e)/8500/t[i]));
  }
  const c=this.current.model;u.uColumnMode.value=c.inputs.morphology.type+1;u.uColumnTime.value=u.uTime.value;
  u.uCover.value=c.inputs.morphology.coverage;u.uTau.value=c.summary.inCloudOpticalDepth;
  if(c.key==='fog'){u.uLocalExtinction.value.addScalar(c.sample(u.uEyeHeight.value).extinction);u.uColumnFogColour.value.copy(u.uDiffuse.value).multiplyScalar(.65/Math.PI).addScaledVector(u.uDirect.value,(.025+.075*Math.exp(-c.summary.inCloudOpticalDepth*.25))/Math.PI);}
 }
 snapshot(){return this.current?{regime:this.key,...this.current.model.summary,optics:{kind:'inherited exponential molecular profile with altitude-dependent RGB cloud transport',...O.profile(this.world),ozoneEnabled:O.profile(this.world).ozone===1,ozoneProfile:'selected 10–40 km triangle; effective RGB coefficients',diffuse:'selected sky/surface two-boundary closure; regional Lambertian albedo 0.18'},time_s:this.atm.uniforms.uColumnTime.value,renderCache:{skyPixels:this.skyTarget?[this.skyTarget.width,this.skyTarget.height]:null,shadowPixels:this.shadowTarget?[this.shadowTarget.width,this.shadowTarget.height]:null,quality:this.quality||'economy',bakes:this.bakeCount,lastSubmission_ms:this.lastBakeMs,windCadence_s:2,positionThreshold_m:32,shadowPlaneAltitude_m:0,shadowExtent_m:32768}}:null;}
 initRenderer(renderer,prelude){
  const T=this.T;this.renderer=renderer;
  const options={type:T.HalfFloatType,depthBuffer:false,minFilter:T.LinearFilter,magFilter:T.LinearFilter};
  ...
  this.skyTarget=new T.WebGLRenderTarget(768,384,options);this.skyTarget.texture.wrapS=T.RepeatWrapping;
  this.shadowTarget=new T.WebGLRenderTarget(128,128,options);
  const u=this.atm.uniforms;u.uColumnSkyMap.value=this.skyTarget.texture;u.uColumnShadowMap.value=this.shadowTarget.texture;
  const vertex='varying vec2 vUV;void main(){vUV=uv;gl_Position=vec4(position.xy,0.,1.);}';
  this.bakeScene=new T.Scene();this.bakeCamera=new T.OrthographicCamera(-1,1,1,-1,0,1);
  this.skyBakeMaterial=new T.ShaderMaterial({uniforms:u,vertexShader:vertex,fragmentShader:prelude+DENSITY_GLSL+SKY_GLSL+`
   varying vec2 vUV;
  ...
  this.shadowBakeMaterial=new T.ShaderMaterial({uniforms:u,vertexShader:vertex,fragmentShader:prelude+DENSITY_GLSL+`
   varying vec2 vUV;
  ...
  `,toneMapped:false,depthTest:false,depthWrite:false});
  this.bakeMesh=new T.Mesh(new T.PlaneGeometry(2,2),this.skyBakeMaterial);this.bakeScene.add(this.bakeMesh);
 }
 setQuality(quality){
  if(!['economy','balanced','high'].includes(quality))throw new RangeError('Unknown cloud quality');
  const width=quality==='high'?3072:quality==='balanced'?1536:768;
  this.quality=quality;
  if(this.skyTarget&&this.skyTarget.width!==width){this.skyTarget.setSize(width,width/2);this.renderKey=null;this.atm.clearEnvKey=null;this.atm.envTime=-1e9;}
 }

 prepare(renderer){
  const u=this.atm.uniforms;if(this.key==='reference'||!this.current||!this.skyTarget)return false;
  const time=Math.floor(u.uTime.value/2)*2,observer=u.uObserver.value;
  const key=[this.world,this.key,u.uBlend.value,u.uSide.value,...u.uSun.value.toArray(),time,Math.floor(observer.x/32),Math.floor(observer.y/32),Math.round(u.uEyeHeight.value)].join(':');
  if(key===this.renderKey)return false;
  const started=performance.now(),oldTarget=renderer.getRenderTarget(),tm=renderer.toneMapping,exposure=renderer.toneMappingExposure,oldTime=u.uColumnTime.value;
  const centreX=Math.round(observer.x/512)*512,centreZ=Math.round(observer.y/512)*512;
  u.uColumnShadowBounds.value.set(centreX-16384,centreZ-16384,32768,128);
  u.uColumnAverageShadow.value=1.-this.current.model.inputs.morphology.coverage*(1.-Math.exp(-this.current.model.summary.inCloudOpticalDepth/Math.max(.1,u.uSun.value.y)));
  u.uColumnTime.value=time;u.uColumnCacheReady.value=0;u.uColumnShadowReady.value=0;
  try{
   renderer.toneMapping=this.T.NoToneMapping;renderer.toneMappingExposure=1;
   ...
   const sky=this.skyTarget;sky.scissorTest=true;
   for(let y=0;y<sky.height;y+=128)for(let x=0;x<sky.width;x+=256){
    sky.scissor.set(x,y,Math.min(256,sky.width-x),Math.min(128,sky.height-y));renderer.setRenderTarget(sky);renderer.render(this.bakeScene,this.bakeCamera);
   }
   sky.scissorTest=false;
   this.bakeMesh.material=this.shadowBakeMaterial;renderer.setRenderTarget(this.shadowTarget);renderer.render(this.bakeScene,this.bakeCamera);
   u.uColumnCacheReady.value=1;u.uColumnShadowReady.value=1;this.renderKey=key;this.bakeCount++;
  }finally{renderer.setRenderTarget(oldTarget);renderer.toneMapping=tm;renderer.toneMappingExposure=exposure;u.uColumnTime.value=oldTime;this.lastBakeMs=performance.now()-started;}
  return true;
 }

 dispose(){this.noiseTexture.dispose();this.profileTexture.dispose();this.sunTexture.dispose();this.cache.clear();this.skyTarget?.dispose();this.shadowTarget?.dispose();this.skyBakeMaterial?.dispose();this.shadowBakeMaterial?.dispose();this.bakeMesh?.geometry.dispose();}
}
const API={noiseLattice,UNIFORMS,DENSITY_GLSL,SHADOW_GLSL,SKY_GLSL,CACHED_SKY_GLSL,Controller};root.OpenMoonCloudRenderer=API;if(typeof module!=='undefined')module.exports=API;
})(globalThis);
