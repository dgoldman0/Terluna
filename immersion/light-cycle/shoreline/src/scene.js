...
function animateDepth(T,o,uniforms,sway){if(!sway)return;const d=new T.MeshDepthMaterial({depthPacking:T.RGBADepthPacking,side:T.DoubleSide});d.onBeforeCompile=s=>{Object.assign(s.uniforms,uniforms);s.vertexShader='uniform float uTime,uWind,uSway;\n'+s.vertexShader;s.vertexShader=s.vertexShader.replace('#include <begin_vertex>',`#include <begin_vertex>
 ...
 p=modelMatrix*p;transformed.x+=min(1.,max(0.,position.y*position.y))*uSway*sin(p.x*.36+p.z*.16+uTime*(.65+uWind*.035))*min(1.5,uWind*.18);transformed.z+=min(1.,max(0.,position.y*position.y))*uSway*.45*sin(p.x*.18-p.z*.28+uTime*.77)*min(1.5,uWind*.18);`);};o.customDepthMaterial=d;}
function createScene(T,scene,atm,world='moon'){
 ...
 const S=C.SHELTER;let deckY=C.groundHeight(S.x,S.z,world)+.22;state.pavilionY=deckY;
 const deckMat=OM.material(T,atm,state,'timber'),frameMat=new T.MeshStandardMaterial({color:0x313e39,metalness:.3,roughness:.5}),roofMat=new T.MeshStandardMaterial({color:0x616960,metalness:.35,roughness:.45});
 ...
 for(let i=0;i<rp.count;i++){const x=rp.getX(i)+S.x,z=rp.getZ(i)+S.z-7.5;rp.setXYZ(i,x,C.mix(C.groundHeight(x,z,world),deckY,C.smooth(0,1,(z-S.z+11)/7)),z);}ramp.computeVertexNormals();const rampMesh=mesh(T,ramp,deckMat,scene);rampMesh.userData.surfaceConforming=true;
 ...
 const baseY=new Map(),treeY=state.trees.map(t=>t.y),baseDeckY=deckY;
 for(const o of decorations){
  if(o.isInstancedMesh)baseY.set(o,Float32Array.from({length:o.count},(_,i)=>o.instanceMatrix.array[i*16+13]));
  else if(o.userData.surfaceConforming)baseY.set(o,Float32Array.from({length:o.geometry.attributes.position.count},(_,i)=>o.geometry.attributes.position.getY(i)));
  else baseY.set(o,o.position.y);
 }
 state.updateTerrain=(x,z,worldKey)=>{
  if(worldKey!==state.world){
   const delta=(x,z)=>C.curvatureSag(x,z,world)-C.curvatureSag(x,z,worldKey),m=new T.Matrix4();
   for(const o of decorations){
    if(o.isInstancedMesh){for(let i=0;i<o.count;i++){o.getMatrixAt(i,m);m.elements[13]=baseY.get(o)[i]+delta(m.elements[12],m.elements[14]);o.setMatrixAt(i,m);}o.instanceMatrix.needsUpdate=true;o.computeBoundingSphere();}
    else if(o.userData.surfaceConforming){const p=o.geometry.attributes.position;for(let i=0;i<p.count;i++)p.setY(i,baseY.get(o)[i]+delta(p.getX(i),p.getZ(i)));p.needsUpdate=true;o.geometry.computeVertexNormals();o.geometry.computeBoundingSphere();}
...
OM.createScene=createScene;OM.prepareMaterial=prepareMaterial;OM.animateDepth=animateDepth;
})(globalThis);
