...
(function(root){
'use strict';const OM=root.OM=root.OM||{},L=root.OpenMoonLandscape;
OM.loadSurfaceAssets=async function(T){
 const spec=JSON.parse(document.getElementById('surface-assets').textContent),loader=new T.TextureLoader();
 ...
 const makeArray=(image,colourSpace)=>{
  ...
  for(const layer of layers){const x=(layer.layer%layout[0])*(size+2*pad)+pad,y=Math.floor(layer.layer/layout[0])*(size+2*pad)+pad;bytes.set(ctx.getImageData(x,y,size,size).data,layer.layer*size*size*4);}
  const t=new T.DataArrayTexture(bytes,size,size,layers.length);
  t.colorSpace=colourSpace;t.wrapS=t.wrapT=T.RepeatWrapping;t.generateMipmaps=true;t.minFilter=T.LinearMipmapLinearFilter;t.magFilter=T.LinearFilter;t.anisotropy=8;t.unpackAlignment=1;t.needsUpdate=true;
  return t;
 };
 OM.surfaceAssets={albedo:makeArray(albedo.image,T.SRGBColorSpace),packed:makeArray(packed.image,T.NoColorSpace),manifest:spec.manifest};
 albedo.dispose();packed.dispose();
};
...
OM.material=function(T,atm,state,kind,options={}){
 const leaf=kind==='leaf'||kind==='grass',mat=new T.MeshStandardMaterial({color:0xffffff,roughness:.9,...options});
 OM.prepareMaterial(T,mat,atm,state,{ground:kind==='terrain',sway:kind==='leaf'?.015:kind==='grass'?.025:0});
 const previous=mat.onBeforeCompile;
 mat.onBeforeCompile=shader=>{
  previous(shader);shader.fragmentShader=shader.fragmentShader.replace('mix(1.,.58,vOMCanopy)','mix(1.,.68,habitat.g)');Object.assign(shader.uniforms,state.fieldUniforms);
  shader.vertexShader='varying vec2 vOMTex;varying vec2 vOMSize;'+(kind==='terrain'?'attribute vec4 omWeights;varying vec4 vOMWeights;':'')+'\n'+shader.vertexShader;
  ...
  shader.fragmentShader='varying vec2 vOMTex;varying vec2 vOMSize;'+(kind==='terrain'?'varying vec4 vOMWeights;':'')+'\n'+OM.FIELD_GLSL+shader.fragmentShader;
  let detail='';
  if(kind==='terrain')detail=`
  ...
  shader.fragmentShader=shader.fragmentShader.replace(oldWet,`float omWet=spatialWet*(1.-max(omRoof,uProtected));roughnessFactor=mix(microRough,max(.055,microRough*.20),omWet);diffuseColor.rgb*=mix(1.,.72,omWet);`);
 ...
 mat.customProgramCacheKey=()=>`open-moon-landscape-2-${kind}`;return mat;
};
})(globalThis);
