...
// The column thermodynamics belong to the atmosphere domain; the optical integration is this program's.
const W=require('../../../../atmosphere/column/weather-column.js'),A=require('../../../../atmosphere/column/atmospheric-profile.js'),O=require('../../src/profile-optics.js');
const root=path.resolve(__dirname,'../..'),repo=path.resolve(root,'../..'),out=path.resolve(process.argv[2]||path.join(root,'data/a1/generated'));
(async()=>{
 fs.mkdirSync(out,{recursive:true});const manifest={schema:'open-moon-a1-profile-manifest/1',profiles:[],sources:{}};
 // Source identities are repository-relative so the validator can re-hash them wherever they live.
 for(const f of['atmosphere/column/weather-column.js','atmosphere/column/atmospheric-profile.js','illumination/references/src/profile-optics.js'])manifest.sources[f]=crypto.createHash('sha256').update(fs.readFileSync(path.join(repo,f))).digest('hex');
