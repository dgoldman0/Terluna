...
| September feasibility package | Original ZIP available locally; code and outputs inspectable | Verbatim model and all nine CSV tables imported, plus selected original reference fields in JSON. Full historical report, builder and figures stay in source archive; every member is indexed. |
| September protection package | Original ZIP available locally; code, inputs and outputs inspectable | Verbatim model/verifier and compact tables imported, plus selected original reference fields in JSON. Five external inputs pinned and restorable. Large grids and reports are indexed but omitted. |
