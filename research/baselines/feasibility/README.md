...
[report.md](report.md) is the package's full report, imported verbatim on 2026-09-26 with the four figures it shows (`results/`). It records the reasoning behind these tables: the energy-limited loss with no filter, the warning that propulsive stationkeeping can consume more mass than replenishing the air, arrival heat from bulk delivery, and the architecture and schedule gates. The PDF builder stays in the source archive, indexed in [archive manifests](../../provenance.json). External scholarly sources still require complete reading and claim-level admission.
