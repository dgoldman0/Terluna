...
# B1 renders with the A1-A3 reference transport, which lives with the
# illumination domain; its inputs keep their recorded relative names.
REFERENCES=ROOT.parents[1]/'illumination'/'references'
def locate(name):
    """Resolve a recorded relative name (e.g. 'data/a1/...') to its current file."""
    return (REFERENCES if name.split('/')[0] in ('data','tools') and name.split('/')[1] in ('a1','a2','a3') else ROOT)/name
sys.path.insert(0,str(REFERENCES/'tools/a3'))
