...
# The clear-sky atlases, their check record and the notice for the transcribed
# solar/ozone arrays belong to the illumination domain's sky solver.
SKY=ROOT.parents[1]/'illumination'/'sky'

def build():
 worlds={};angles=None;packing=[]
 for key in ['earth','moon','moon_no_ozone']:
  p=SKY/'data'/f'{key}_atlas.npz';d=np.load(p);rgb=d['rgb'].astype(float);n,h,w,_=rgb.shape
 ...
 checks_path=SKY/'validation'/'full_cycle_checks.json';checks=json.loads(checks_path.read_text()) if checks_path.exists() else {}
 ...
 text=(ROOT/'cycle.template.html').read_text();text=text.replace('__VERTEX__',(ROOT/'scene.vert').read_text()).replace('__FRAGMENT__',(ROOT/'scene.frag').read_text()).replace('__APP__',(ROOT/'cycle.js').read_text()).replace('__LICENSES__',html.escape((SKY/'LICENSES.txt').read_text())).replace('__DATASET__',json.dumps(obj,separators=(',',':')))
