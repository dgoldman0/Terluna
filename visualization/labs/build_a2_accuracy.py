...
GEN=REFERENCES/'data/a2/generated'
def load(p):return json.loads(Path(p).read_text())
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def build(output):
 val=load(REFERENCES/'A2_VALIDATION.json')
 ...
 data={'sky':sky,'fine':fine,'cloud':cloud,'captures':caps,'validation':val,'validationSha':sha(REFERENCES/'A2_VALIDATION.json')}
